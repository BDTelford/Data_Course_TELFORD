...
setwd("C:/Users/Ben Telford/Desktop/Data_Course/data/")


list.files(recursive = TRUE, pattern = "thatch_ant.csv")
...
#tidying the data for headwidth column
...
colo = dat[1:20,"Colony"]
mass1 =dat[1:20,"Mass"]
data.frame(Colony = colo, Mass = mass1)
dat3 = data.frame(Colony = colo, Mass = mass1)
...
getwd()
write.csv(dat3, '../../Data_Course_TELFORD/Assignments/Assignment_3/TELFORD_first_file.csv')
...
#fill it in
size_class_mean_mass = data.frame(Factor_levels = levels((dat$Size.class)), Mean = new_vector)
...
palette(col.rainbow)
family <- as.factor(dat$Colony)
plot(dat$Headwidth,dat$Mass,pch=19, col=family, main = "Head_Width_vs_Mass", xlab = "Head_Width", ylab = "Mass", xlim=range(1:30)) 
legend("bottomright", legend=levels(family), pch=16, col=unique(family), title = "Colony")

# 2.  Write the code to save it (with meaningful labels) as a jpeg file
jpeg('../../Data_Course_TELFORD/Assignments/Assignment_3/Headwidth_vs_Mass.jpg')
col.rainbow <- rainbow(12)
palette(col.rainbow)
family <- as.factor(dat$Colony)
plot(dat$Headwidth,dat$Mass,pch=19, col=family, main = "Head_Width_vs_Mass", xlab = "Head_Width", ylab = "Mass", xlim=range(1:30)) 
...
newdata <- subset(dat, Colony < 3)
print(newdata)

# 4.  Write code to save this new subset as a .csv file

write.csv(newdata, '../../Data_Course_TELFORD/Assignments/Assignment_3/TELFORD_ants_first_two_colonies.csv')
